<body>
    <h2>Giải phương trình bậc nhất, 1 ẩn số</h2>
<?php
$t1=1;
$a1=2;
$b1=3;
if($a1 != 0){
    $x1=-$b1/$a1;
    $y1="<p>Phương trình có nghiệm</p>";
}else{
    $t1="<p style='color:red'>$t1</p>";
    $a1="<p style='color:red'>$a1</p>";
    $b1="<p style='color:red'>$b1</p>";
    $x1="<p style='color:red'>-</p>";
    $y1="<p style='color:red'> Hệ số không hợp lệ, a=0 </p>";
}
?>


<?php
$t2=2;
$a2=0;
$b2=3;
if($a2 != 0){
    $x2=-$b2/$a2;
    $y2="<p>Phương trình có nghiệm</p>";
}else{
    $t2="<p style='color:red'>$t2</p>";
    $a2="<p style='color:red'>$a2</p>";
    $b2="<p style='color:red'>$b2</p>";
    $x2="<p style='color:red'> -</p>";
    $y2="<p style='color:red'>Hệ số không hợp lệ, a=0</p>";
}
?>


<?php
$t3=3;
$a3=2;
$b3=3;
if($a3 != 0){
    $x3=-$b3/$a3;
    $y3="<p>Phương trình có nghiệm</p>";
}else{
    $t3="<p style='color:red'>$t3</p>";
    $a3="<p style='color:red'>$a3</p>";
    $b3="<p style='color:red'>$b3</p>";
    $x3="<p style='color:red'>-</p>";
    $y3="<p style='color:red'> Hệ số không hợp lệ, a=0 </p>";
}
?>


<?php
$t4=4;
$a4=2;
$b4=3;
if($a3 != 0){
    $x4=-$b4/$a4;
    $y4="<p>Phương trình có nghiệm</p>";
}else{
    $t4="<p style='color:red'>$t4</p>";
    $a4="<p style='color:red'>$a4</p>";
    $b4="<p style='color:red'>$b4</p>";
    $x4="<p style='color:red'>-</p>";
    $y4="<p style='color:red'> Hệ số không hợp lệ, a=0 </p>";
}

?>


<?php
$t5=5;
$a5=2;
$b5=3;
if($a5 != 0){
    $x5=-$b5/$a5;
    $y5="<p>Phương trình có nghiệm</p>";
}else{
    $t5="<p style='color:red'>$t5</p>";
    $a5="<p style='color:red'>$a5</p>";
    $b5="<p style='color:red'>$b5</p>";
    $x5="<p style='color:red'>-</p>";
    $y5="<p style='color:red'> Hệ số không hợp lệ, a=0 </p>";
}

?>

<?php
$t6=6;
$a6=2;
$b6=3;
if($a6 != 0){
    $x6=-$b6/$a6;
    $y6="<p>Phương trình có nghiệm</p>";
}else{
    $t6="<p style='color:red'>$t6</p>";
    $a6="<p style='color:red'>$a6</p>";
    $b6="<p style='color:red'>$b6</p>";
    $x6="<p style='color:red'>-</p>";
    $y6="<p style='color:red'> Hệ số không hợp lệ, a=0 </p>";
}

?>

<?php
$t7=7;
$a7=2;
$b7=3;
if($a7 != 0){
    $x7=-$b7/$a7;
    $y7="<p>Phương trình có nghiệm</p>";
}else{
   $t7="<p style='color:red'>$t7</p>";
    $a7="<p style='color:red'>$a7</p>";
    $b7="<p style='color:red'>$b7</p>";
    $x7="<p style='color:red'>-</p>";
    $y7="<p style='color:red'> Hệ số không hợp lệ, a=0 </p>";
}

?>

<?php
$t8=8;
$a8=2;
$b8=3;
if($a8 != 0){
    $x8=-$b8/$a8;
    $y8="<p>Phương trình có nghiệm</p>";
}else{
    $t8="<p style='color:red'>$t8</p>";
    $a8="<p style='color:red'>$a8</p>";
    $b8="<p style='color:red'>$b8</p>";
    $x8="<p style='color:red'>-</p>";
    $y8="<p style='color:red'> Hệ số không hợp lệ, a=0 </p>";
}

?>

<?php
$t9=9;
$a9=2;
$b9=3;
if($a9 != 0){
    $x9=-$b9/$a9;
    $y9="<p>Phương trình có nghiệm</p>";
}else{
    $t9="<p style='color:red'>$t9</p>";
    $a1="<p style='color:red'>$a9</p>";
    $b9="<p style='color:red'>$b9</p>";
    $x9="<p style='color:red'>-</p>";
    $y9="<p style='color:red'> Hệ số không hợp lệ, a=0 </p>";
}

?>

<?php
$t10=10;
$a10=2;
$b10=3;
if($a10 != 0){
    $x10=-$b10/$a10;
    $y10="<p>Phương trình có nghiệm</p>";
}else{
   $t10="<p style='color:red'>$t10</p>";
    $a10="<p style='color:red'>$a10</p>";
    $b10="<p style='color:red'>$b10</p>";
    $x10="<p style='color:red'>-</p>";
    $y10="<p style='color:red'> Hệ số không hợp lệ, a=0 </p>";
}

?>


    <table border="1px" cellpadding="5px" cellspacing="0px"style="width:50%;">
        <tr bgcolor=#cccccc>
            <th style="text-align: center;">TT</th>
            <th style="text-align: right;">a</th>
            <th style="text-align: right;">b</th>
            <th style="text-align: right;">x</th>
            <th style="text-align: center;">Ghi chú</th>
        </tr>
        <tr>
            <td style="text-align: center;"><?= $t1 ?></td>
             <td style="text-align: right;"><p><?= $a1 ?></p></td>
             <td style="text-align: right;"><p><?= $b1 ?></p></td>
             <td style="text-align: right;"><p><?= $x1 ?></p></td>
             <td style="text-align: left;"><p><?= $y1 ?></p></td>
        </tr>
        <tr>
            <td style="text-align: center;"><?=$t2?></td>
             <td style="text-align: right;"><p><?=$a2?></p></td>
             <td style="text-align: right;"><p><?=$b2?></p></td>
             <td style="text-align: right;"><p><?=$x2?></p></td>
             <td style="text-align: left;"><p><?=$y2?></p></td>
        </tr>
        <tr>
            <td style="text-align: center;">3</td>
             <td style="text-align: right;"><p><?=$a3?></p></td>
             <td style="text-align: right;"><p><?=$b3?></p></td>
             <td style="text-align: right;"><p><?=$x3?></p></td>
             <td style="text-align: left;"><p><?=$y3?></p></td>
        </tr>
        <tr>
            <td style="text-align: center;">4</td>
             <td style="text-align: right;"><p><?=$a4?></p></td>
             <td style="text-align: right;"><p><?=$b4?></p></td>
             <td style="text-align: right;"><p><?=$x4?></p></td>
             <td style="text-align: left;"><p><?=$y4?></p></td>
        </tr>
        <tr>
            <td style="text-align: center;">5</td>
             <td style="text-align: right;"><p><?=$a5?></p></td>
             <td style="text-align: right;"><p><?=$b5?></p></td>
             <td style="text-align: right;"><p><?=$x5?></p></td>
             <td style="text-align: left;"><p><?=$y5?></p></td>
        </tr>
        <tr>
            <td style="text-align: center;">6</td>
             <td style="text-align: right;"><p><?=$a6?></p></td>
             <td style="text-align: right;"><p><?=$b6?></p></td>
             <td style="text-align: right;"><p><?=$x6?></p></td>
             <td style="text-align: left;"><p><?=$y6?></p></td>
        </tr>
        <tr>
            <td style="text-align: center;">7</td>
             <td style="text-align: right;"><p><?=$a7?></p></td>
             <td style="text-align: right;"><p><?=$b7?></p></td>
             <td style="text-align: right;"><p><?=$x7?></p></td>
             <td style="text-align: left;"><p><?=$y7?></p></td>
        </tr>
        <tr>
            <td style="text-align: center;">8</td>
             <td style="text-align: right;"><p><?=$a8?></p></td>
             <td style="text-align: right;"><p><?=$b8?></p></td>
             <td style="text-align: right;"><p><?=$x8?></p></td>
             <td style="text-align: left;"><p><?=$y8?></p></td>
        </tr>
        <tr>
            <td style="text-align: center;">9</td>
             <td style="text-align: right;"><p><?=$a9?></p></td>
             <td style="text-align: right;"><p><?=$b9?></p></td>
             <td style="text-align: right;"><p><?=$x9?></p></td>
             <td style="text-align: left;"><p><?=$y9?></p></td>
        </tr>
        <tr>
            <td style="text-align: center;">10</td>
             <td style="text-align: right;"><p><?=$a10?></p></td>
             <td style="text-align: right;"><p><?=$b10?></p></td>
             <td style="text-align: right;"><p><?=$x10?></p></td>
             <td style="text-align: left;"><p><?=$y10?></p></td>
        </tr>
        <?php
    if($a1==0 && $a2==0){
        
    }
        ?>
    </table>
</body>